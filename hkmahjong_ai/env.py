...
from .hand_eval import can_win, chow_options, classify_win, winning_with_tile
...
class HKMahjongEnv:
    ...
    def declare_self_win(self, player_id: int | None = None) -> GameResult:
        ...
        return self._finish(pid, None, "self_draw", f"P{pid + 1} 自摸", pattern)
    ...
    def _resolve_claims(
        ...
        if win_candidates:
            ...
            counts[tile] += 1
            pattern = classify_win(counts, len(player.melds))
            return (
                self._finish(
                    winner,
                    discarder,
                    "discard_win",
                    f"P{winner + 1} 食糊 {tile_name(tile)}",
                    pattern,
                ),
                False,
            )
        ...
        for preferred_kind in ("kong", "pong", "chow"):
    ...
    def _finish(
        ...
        self.done = True
        self.result = GameResult(winner, loser, kind, self.turns, len(self.wall), message, pattern)
        self._log(message)
        return self.result
