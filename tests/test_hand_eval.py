...
from hkmahjong_ai.hand_eval import _can_form_melds, can_win, hand_potential, is_seven_pairs, winning_with_tile
...
def test_invalid_tile_count_is_not_a_win() -> None:
    ...
    assert not can_win([14])
    assert not winning_with_tile([0] * TILE_COUNT, TILE_COUNT)


def test_hand_potential_values_internal_sequences_even_without_chow_claims() -> None:
